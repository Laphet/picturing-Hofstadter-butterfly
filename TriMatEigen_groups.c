...
#pragma omp parallel for
        for (i = 0; i < groups; ++i)
        {
            ...
            {
                ctx_local = (Context){
                    .order = k + r, .low_bound = ctx->low_bound, .up_bound = ctx->up_bound, .tol = -1.0, .alpha = &(ctx->alpha[0]), .beta = &(ctx->beta[0])};
            }
            else
            {
                ctx_local = (Context){
                    .order = k, .low_bound = ctx->low_bound, .up_bound = ctx->up_bound, .tol = -1.0, .alpha = &(ctx->alpha[i * k + r - 1]), .beta = (i * k + r - 1 < ctx->order - 1) ? &(ctx->beta[i * k + r - 1]) : NULL};
            }
